...
- **Perfil** (`Perfil.tsx`): única tarjeta "transparente" del sitio — fondo `rust/10` con `backdrop-blur-sm` y borde `rust/20` (un tinte cálido apenas visible sobre el navy, no un `bg-cream` sólido como el resto de las tarjetas), para leerse como una pieza aparte y no como una sección más. Foto a la izquierda (arriba y centrada en mobile) junto al nombre completo (`persona.fullName`) como encabezado y un único párrafo de resumen (`resumenProfesional`) en itálica sin negrita, más pequeño que el nombre. Se renderiza sin animación de entrada a propósito: al vivir tan cerca del fold (justo después del Hero, con el padding recortado — ver nota de espaciado abajo), un fundido con opacidad inicial en 0 lo convertía en el candidato a Largest Contentful Paint y ese fundido gated por `IntersectionObserver` disparaba el LCP a ~9s bajo Lighthouse — visible instantáneo desde el load, igual que el Hero.
...
- **Reconocimiento Institucional** (`ReconocimientoInstitucional.tsx` + `ReconocimientoModal.tsx`): los tres reconocimientos se ven uniformes — un badge circular con un ícono SVG propio (dibujado a mano, sin librería de íconos) y una descripción corta debajo, los tres en una fila centrada. El color de cada badge usa `rust-light`/`sage-light`/`cream` (no las variantes `-ink`: contra `navy` esas solo llegan a ~2.4:1, por debajo del mínimo 3:1 para contraste no-textual). Al hacer clic se abre un modal (fundido + escala vía estado de React, no CSS `@keyframes`, igual que el resto del sitio) que muestra el contenido real: el iframe de LinkedIn para los dos reconocimientos sociales, o una vista previa en iframe del PDF de la constancia para el tercero — así no hace falta salir de la página. El modal maneja foco (se mueve al botón de cerrar al abrir, regresa al badge que lo abrió al cerrar), cierra con Escape o clic en el fondo, y bloquea el scroll del body mientras está abierto. Nota de implementación: usa `setTimeout` en vez de `requestAnimationFrame` para disparar la transición de entrada — `requestAnimationFrame` se pausa por completo en pestañas en segundo plano, y un timer corto no.
...
**Espaciado entre secciones:** el padding vertical estándar de cada `<section>` es `py-12 sm:py-16` (antes `py-16 sm:py-24`, y el Hero llegaba a `py-24 sm:py-32`) — se redujo tras feedback de que el sitio se sentía "poco fluido", con demasiado aire entre el final de una tarjeta y el encabezado siguiente.
